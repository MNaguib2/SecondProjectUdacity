...
exports.RemoveItemToCredit = exports.ShowCredit = exports.AddItemToCredit = void 0;
...
function AddItemToCredit(req, res, next) {
    ...
    if (errorMessage.length < 1) {
        ...
        const quantity = 1;
        new Credit_model_1.CrediteModel().getAllCrediteToUser(idUser)
            .then(result => {
            if (result) {
                const idcared = result.id;
                new Credit_model_1.CrediteModel().getCreditItem(idItem, idcared)
                    .then(items => {
                    if (items) {
                        ...
                        new Credit_model_1.CrediteModel().AddItemAgain(TotalQuantity, totalprice, items.id)
                            .then(sucessfuladd => {
                            if (sucessfuladd) {
                                new Credit_model_1.CrediteModel().CalculateTotalPrice(idcared);
                                res.status(200).json({
                        ...
                        new Product_model_1.ProductModel().getProductByID(idItem)
                            .then(product => {
                            if (product) {
                                new Credit_model_1.CrediteModel().AddItemCredit({
                ...
                new Product_model_1.ProductModel().getProductByID(idItem)
                    .then(Product => {
                    if (Product) {
                        new Credit_model_1.CrediteModel().CreateNewCredit({ id: 0, id_user: idUser, totalprice: Product.price })
                            .then(CreateNewID => {
                            if (CreateNewID > 0) {
...
function ShowCredit(req, res, next) {
    ...
    new Credit_model_1.CrediteModel().getAllCrediteToUser(idUser)
        .then(credits => {
        if (credits) {
            new Credit_model_1.CrediteModel().getALLCreditItem(credits.id)
                .then(items => {
                if (items.length > 0) {
                    res.status(200).json({
...
function RemoveItemToCredit(req, res, next) {
    ...
    if (errorMessage.length < 1) {
        ...
        new Credit_model_1.CrediteModel().getAllCrediteToUser(idUser)
            .then(Cards => {
            if (Cards) {
                new Credit_model_1.CrediteModel().DeleteItem(Cards.id, idItem)
                    .then(Deleted => {
                    if (Deleted > 0) {
