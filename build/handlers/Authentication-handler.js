        ...
        new User_model_1.UserModel().getAll()
            ...
            if (result.length > 0) {
                ...
                if (found) {
                    ...
                    new User_model_1.UserModel().AddUser({ name: name, email: email, password: hashOassword, typeuser: 3, statue: 0, id: 0, token: null })
                        .then(added => {
                ...
                const hashOassword = bcrybt.hashSync(password, 12);
                new User_model_1.UserModel().AddAdmin({ name: name, email: email, password: hashOassword,
    ...
    if (errorMessage.length < 1) {
        ...
        new User_model_1.UserModel().FindUserByEmail(email)
            ...
            if (result && bcrybt.compareSync(password, result.password)) {
                ...
                const token = jwt.sign({ Token: TokenGen, email: result.email }, process.env.PassWordDev, { expiresIn: Date.now() - (3600000 * 458) });
                new User_model_1.UserModel().AddUserToken(TokenGen, result.id)
                    .then(() => {
