...
exports.CridetMiddleWare = exports.ClientMiddleWare = void 0;
...
function CridetMiddleWare(req, res, next) {
  const token = req.get('Token');
  jwt.verify(token, process.env.PassWordDev, (err, decoded) => {
    if (err) {
      res.status(401).json({
    ...
    const decodedPayload = decoded;
    if (Date.now() < decodedPayload.exp) {
      // console.log(new Date(decodedPayload.exp as number));
...
exports.CridetMiddleWare = CridetMiddleWare;
