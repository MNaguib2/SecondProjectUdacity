...
export class CrediteModel {
  async getAllCrediteToUser(id: number): Promise<Credite> {
    try {
      ...
      throw new Error(
        `Cannot get Credite specialist to user from DB error 15 ${err}`
      );
    }
  }
  async getCreditItem(idProd: number, idCard: number): Promise<CrediteItem> {
    try {
      const conn = await client.connect();
      const sql =
        'select * from carditem where id_product = ' +
        idProd +
        'AND id_card = ' +
        idCard;
      const result = await conn.query(sql);
      ...
      throw new Error(
        `Cannot get Credite specialist item to cardid from DB error 21 ${err}`
      );
    }
  }
  async getALLCreditItem(idCard: number): Promise<CrediteItem[]> {
    try {
      ...
      throw new Error(
        `Cannot get Credite all item to cardid from DB error 29 ${err}`
      );
    }
  }
  ...
  async DeleteItem(idCard: number, idProd: number): Promise<number> {
    try {
      ...
      throw new Error(
        `Cannot Deleted Item From Card think not item in this card 28 ${err}`
      );
    }
  }
}
