...
export function logoutHandler(req: Request, res: Response, next: NextFunction) {
  new UserModel()
    .FindUserByEmail((req as IGetUserAuthInfoRequest).user.email)
    .then((user) => {
      if (user) {
        if (user && user.token) {
          new UserModel()
            ...
            .then((Update) => {
              if (Update === 'UPDATE') {
                ...
                const error = new Error(
                  'Occur Error! During Excute Function Logout error 13'
                );
                error.name = '406 ';
                throw error;
              }
          ...
          const error = new Error(
            'Occur Error! this user Or Token Invalid  error 14'
          );
          error.name = '401 ';
          throw error;
        }
