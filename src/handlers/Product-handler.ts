        ...
        .then((result) => {
          if (result === 'INSERT') {
            ...
            const error = new Error(
              'Occur Error! Unexpected Please Call Developer'
            );
            error.name = '404';
            throw error;
          }
...
export function EditProduct(req: Request, res: Response, next: NextFunction) {
  ...
  new ProductModel()
    ...
    .then((product) => {
      if (
        product &&
        (product.id_user === (req as IGetUserAuthInfoRequest).user.id ||
          (req as IGetUserAuthInfoRequest).user.typeuser === 1)
      ) {
        const name = req.body.name;
        const description = req.body.description;
        const price = req.body.price;
        new ProductModel()
          ...
          .then((result) => {
            if (result === 'UPDATE') {
              ...
              const error = new Error(
                'Occur Error! During Excute Function Update Product error 8'
              );
              error.name = '406 ';
              throw error;
            }
        ...
        const error = new Error(
          'Occur Error! User UN Authenticated to edite in this product error 5'
        );
        error.name = '401 ';
        throw error;
      }
...
export function DeleteProduct(req: Request, res: Response, next: NextFunction) {
  ...
  new ProductModel()
    ...
    .then((result) => {
      if (
        result &&
        (result.id_user === (req as IGetUserAuthInfoRequest).user.id ||
          (req as IGetUserAuthInfoRequest).user.typeuser == 1)
      ) {
        new ProductModel()
          .DeleteProductByID(Id_Product)
          .then((Deleted) => {
            if (Deleted === 'DELETE') {
              res.status(200).json({
              ...
              const error = new Error(
                'Occur Error! During Excute Function Deleted Product error 9'
              );
              error.name = '406 ';
              throw error;
            }
        ...
        const error = new Error(
          'Occur Error! User UN Authenticated to Delete in this product or Product not found error 30'
        );
        error.name = '401 ';
        throw error;
      }
