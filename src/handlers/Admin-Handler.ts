...
export function changestatueHandler(
  req: Request,
  res: Response,
  next: NextFunction
) {
  const statue: number =
    (req as IGetUserAuthInfoRequest).user.statue == 1 ? 2 : 1;
  new AdminModel()
    .changeStatue(statue, (req as IGetUserAuthInfoRequest).user.id)
    .then((reschangestatue) => {
      if (reschangestatue === 'UPDATE') {
        res.status(200).json({
          message:
            'Successfull Change Update Your statue is ' +
            (statue == 1 ? 'online' : 'offline')
        });
      } else {
        const error = new Error(
          'Occur Error! In change Unexpected Plese return to Develop'
        );
        error.name = '409';
        throw error;
      }
